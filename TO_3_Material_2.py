...
## There are diffrent variants for quick check of final NGnet:

## NGnet1:
# NGnet1 = NGnet("1", sigma, Overlap, G_Din, G_Dout,w1)
# NGnet1 = NGnet("1", 2, 0.3, 20, 100, w1)
NGnet1 = NGnet("1", 1.2, 0.2, 40, 90, w1)
# NGnet1 = NGnet("1", 1.1, 0.25, 38, 90, w1)  # Number_of_Gauss: 78
...
# The final Geometry Matrix is created.
#######################################################################################################################
# Plot of final Geometry Matrix
TitleName = 'Geometry_MATRIX'
fig = plt.figure(figsize=(6.4, 3.5))
fig.canvas.manager.set_window_title(TitleName)
...
#
# Note: Single elements is considert as an element that is not connected
# to another element with same material.

print('Geometry check starts! - Single element check!')

# Checking all materials separetly:
...
for i in range(Nrad):
    for j in range(Nphr):
        ...
        if ((Elements[i][j] == 0) and (i > 0 and j > 0 and i < (Nrad - 1) and j < (Nphr - 1))):
            if (((Elements[i - 1][j] == 0) or (Elements[i + 1][j] == 0) or (Elements[i][j - 1] == 0)
                 or (Elements[i][j + 1] == 0)) != 1):
                Single_element = Single_element + 1
                print('Single element exists!')
        ...
        if ((Elements[i][j] == 1) and (i > 0 and j > 0 and i < (Nrad - 1) and j < (Nphr - 1))):
            if (((Elements[i - 1][j] == 1) or (Elements[i + 1][j] == 1) or (Elements[i][j - 1] == 1)
                 or (Elements[i][j + 1] == 1)) != 1):
                Single_element = Single_element + 1
                print('Single element exists!')
                Elements_flying_part[i][j] = -1
                Elements_show[i][j] = -0.5
...
for i in range(Nrad):
    for j in range(Nphr):
        if Elements[i][j] == Material_Iron:
            Number_of_iron_elements_in_geometry = Number_of_iron_elements_in_geometry + 1

print("Number of iron", Number_of_iron_elements_in_geometry)
